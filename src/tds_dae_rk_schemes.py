...
class TDS_simulation():
    ...
    def update_function(self, states_x0, states_x1) -> torch.Tensor:
        ...
        if self.pinn_boost == 1:
            inputs_pinn = [Vm1_0.clone().detach().item(), Theta1_0.clone().detach().item(), delta1_0.clone().detach().item(), omega1_0.clone().detach().item()]
            self.check_pinn_limits(inputs_pinn)
            pinn_input_data = torch.cat([Vm1_0.view(-1,1), Vm1.view(-1,1), Theta1_0.view(-1,1), Theta1.view(-1,1), delta1_0.view(-1,1), omega1_0.view(-1,1), self.step_size.view(-1,1)], dim=1)
            d_values_pinn = self.pinn_integration_scheme(pinn_input_data)
            res_2 = delta1 - delta1_0 -self.step_size*d_values_pinn[0]
        ...
        if self.pinn_boost == 2:
            inputs_pinn = [Vm2_0.clone().detach().item(), Theta2_0.clone().detach().item(), delta2_0.clone().detach().item(), omega2_0.clone().detach().item()]
            self.check_pinn_limits(inputs_pinn)
            pinn_input_data = torch.cat([Vm2_0.view(-1,1), Vm2.view(-1,1), Theta2_0.view(-1,1), Theta2.view(-1,1), delta2_0.view(-1,1), omega2_0.view(-1,1), self.step_size.view(-1,1)], dim=1)
            d_values_pinn = self.pinn_integration_scheme(pinn_input_data)
            res_12 = delta2 - delta2_0 -self.step_size*d_values_pinn[0]
        ...
        if self.pinn_boost == 3:
            inputs_pinn = [Vm3_0.clone().detach().item(), Theta3_0.clone().detach().item(), delta3_0.clone().detach().item(), omega3_0.clone().detach().item()]
            self.check_pinn_limits(inputs_pinn)
            pinn_input_data = torch.cat([Vm3_0.view(-1,1), Vm3.view(-1,1), Theta3_0.view(-1,1), Theta3.view(-1,1), delta3_0.view(-1,1), omega3_0.view(-1,1), self.step_size.view(-1,1)], dim=1)
            d_values_pinn = self.pinn_integration_scheme(pinn_input_data)
            res_22 = delta3 - delta3_0 -self.step_size*d_values_pinn[0]
    ...
    def newton_method(self, states_x0) -> torch.Tensor:
        ...
        states_x1 = states_x0.clone().requires_grad_(True)
        it_count = 0
        ...
        while torch.max(torch.abs(residual_iteration)) > torch.tensor(1e-6, dtype=torch.float64) and it_count < 10:
            ...
            new_states_x1 = states_x1.detach().clone() + x_increment.detach().clone()

            states_x1 = new_states_x1.clone().requires_grad_(True)

            it_count +=1 
        return states_x1.detach().clone()
    
    def simulation_main_loop(self, integration_scheme='trapezoidal') -> tuple:

        self.integration_scheme = integration_scheme

        states_evolution = self.initial_state
        time_array_compiled = [0.0]
        forward_pass = self.initial_state
        num_iterations_sim = int(self.t_final/self.step_size.item())
        ...
        for i in range(num_iterations_sim):

            result_forward = self.newton_method(forward_pass)

            forward_pass = result_forward

            states_evolution = torch.vstack([states_evolution, result_forward])

            time_array_compiled.append(i*self.step_size.item()+self.step_size.item())

        return time_array_compiled, states_evolution.clone().numpy()
